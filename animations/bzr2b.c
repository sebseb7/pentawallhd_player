...
static uint8_t tick(void) {
	...
	for(int y = 0, p = 0; y < LED_HEIGHT; y++) {
		for (int x = 0; x < LED_WIDTH; x++, p++) {
			...
			int16_t ta = (sa * (266 + sb - sc)) >> 8;
			int16_t tb = (sb * (266 + sc - sa)) >> 8;
			int16_t tc = (sc * (266 + sa - sb)) >> 8;
	...
	for(int y = 0, p = 0; y < LED_HEIGHT; y++) {
		for (int x = 0; x < LED_WIDTH; x++, p++) {
			bzr_a[p] = (t_bzr_a[p] + 2*bzr_a[p]) / 3;
			bzr_b[p] = (t_bzr_b[p] + 2*bzr_b[p]) / 3;
			bzr_c[p] = (t_bzr_c[p] + t_bzr_b[p]) >> 1;
			bzr_c[p] = (t_bzr_a[p]>>4)+(t_bzr_c[p]) * 0.9f;
